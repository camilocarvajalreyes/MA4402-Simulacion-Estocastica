...
\begin{enumerate}
    ...
    Más aun,   disponemos de un  \mathbf{flujo aleatorio}  $(\Phi_{n,m} :E \rightarrow E )_{n\geq 0, m> n} $ dado por  $ \Phi_{n,m} (x)=  \Phi_{m} \circ \cdots \circ \Phi_n(x)$,   que puede  entregar  las transiciones de la cadena desde  cualquier estado $x$  en el tiempo $n-1$, a algún estado $y=\Phi_{n,m} (x)$  en el tiempo $m$.
